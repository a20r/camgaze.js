...
we are able to capture the motion of the pupil without the influence of head
movement or camera movement.

For this work, we use the center of the Haar rectangle that is detected around
the eye. This point will in a constant position with reference to the movement
of the pupil and therefore we are able to determine the gaze of the user.

\subsection{Calibration}

The manner in which we do calibration is aimed to be very easy for a web user
to do. Four corner points are shown to the user one at a time. After the user
looks at the calibration point, the user clicks the screen and the next point
is shown. Using these four points, a linear mapping can be created from the
gaze metric to the screen.
