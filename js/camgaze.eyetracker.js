...
camgaze.EyeTracker.prototype = {
    ...
    getEyeRectangles : function () {
        var left_eye_ids_x = [23, 25];
        var left_eye_ids_y = [24, 26];

        var right_eye_ids_x = [30, 28];
        var right_eye_ids_y = [29, 31];

        var positions = ctracker.getCurrentPosition();

        var getRect = function(ids_x, ids_y) {
            var rect = {
                x: positions[ids_x[0]][0],
                y: positions[ids_y[0]][0],
                width: positions[ids_x[1]][0] - positions[ids_x[0]][0],
                height: positions[ids_y[1]][1] - positions[ids_y[0]][1]
            }

            return rect;
        }

        var left_rect = getRect(left_eye_ids_x, left_eye_ids_y);
        var right_rect = getRect(right_eye_ids_x, right_eye_ids_y);
    ...
    */
    ...
    track : function (imageData, video) {
        ...
        // var eyeRects = this.haarDetector.detectObjects(
        //     video,
        //     2.4, // scale factor
        //     1 // min scale
        // );

        var eyeRects = this.getEyeRectangles();
